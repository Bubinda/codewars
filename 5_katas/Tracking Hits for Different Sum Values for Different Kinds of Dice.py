...
def reg_sum_hits(n, s):
    rolls = range(1, s+1)
    combinations = itertools.product(rolls, repeat=n)
    hits = {}
    ...
    return [[key, value] for key, value in sorted(hits.items())]
...
def reg_sum_hits(n, s):
    e = poly.Polynomial([0] + [1] * s)
    e = e ** n
    return [[i, k] for i, k in enumerate(e.coef) if k != 0]